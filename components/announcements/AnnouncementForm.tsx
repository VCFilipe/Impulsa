...
interface AnnouncementFormProps {
  ...
  onSubmit: (announcement: Omit<Announcement, 'id' | 'createdAt' | 'updatedAt' | 'authorId' | 'authorName'> | Announcement) => void;
  onCancel: () => void;
}

const AnnouncementForm: React.FC<AnnouncementFormProps> = ({ announcementToEdit, onSubmit, onCancel }) => {
  ...
  const [formData, setFormData] = useState<Omit<Announcement, 'id' | 'createdAt' | 'updatedAt' | 'authorId' | 'authorName'>>(initialFormState);
  ...
  const handleImageFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
        if (file.size > 2 * 1024 * 1024) { // 2MB limit
            addToast({ type: 'error', title: 'Arquivo Grande', message: 'A imagem não pode exceder 2MB.' });
  ...
  const handleImproveContent = async () => {
    ...
    if (!formData.content.trim()) {
      addToast({ type: 'info', title: 'Campo Vazio', message: 'Escreva o conteúdo antes de usar a IA para melhorá-lo.' });
      return;
    }
...
Texto refinado:`;
      ...
      if (improvedText && improvedText.trim() && improvedText.trim() !== formData.content.trim()) {
        setFormData(prev => ({ ...prev, content: improvedText.trim() }));
        addToast({ type: 'success', title: 'Conteúdo Melhorado!', message: 'O conteúdo foi refinado pela IA.' });
      } else {
        addToast({ type: 'info', title: 'Sem Mudanças Significativas', message: 'A IA considerou o conteúdo original adequado.' });
      }
    } catch (error) {
      addToast({ type: 'error', title: 'Erro na IA', message: 'Não foi possível melhorar o conteúdo.' });
    } finally {
  ...
  const validate = (): boolean => {
    ...
    if (formData.content.trim().length > 10000) newErrors.content = 'Conteúdo não pode exceder 10000 caracteres.';

    if (!formData.category) newErrors.category = 'Categoria é obrigatória.';
    
    if (formData.imageUrl && !formData.imageUrl.startsWith('data:image/') && !formData.imageUrl.startsWith('http')) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        ...
        <Input
          ...
          error={errors.title} maxLength={150} required
        />
      </div>
      ...
      <div>
        <div className="flex justify-between items-center mb-1">
          ...
          >
            {isImprovingContent ? <Loader2 size={14} className="animate-spin mr-1.5" /> : <Wand2 size={14} className="mr-1.5" />}
            Melhorar
          </Button>
        </div>
        <Textarea
          id="content" name="content" value={formData.content} onChange={handleChange}
          error={errors.content} maxLength={10000} rows={8} required
        />
      </div>
      ...
      <div>
        ...
        <Select
          ...
          options={ANNOUNCEMENT_CATEGORIES.map(cat => ({ value: cat, label: cat }))}
      ...
      <div className="space-y-3 p-4 border border-gray-200 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700">
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Imagem de Destaque (Opcional)</label>
        {imagePreview && (
          <div className="relative group">
            <img src={imagePreview} alt="Preview do comunicado" className="w-full h-40 object-cover rounded-md border border-gray-300 dark:border-gray-500" />
            <Button 
                type="button" 
                variant="danger" 
                size="sm" 
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400">Tamanho máximo: 2MB. Formatos: JPG, PNG, GIF, WebP.</p>
        {errors.imageUrl && <p className="text-xs text-red-600 dark:text-red-400">{errors.imageUrl}</p>}

        <Input
            label="Ou cole uma URL da imagem (Opcional)"
            id="imageUrl" name="imageUrl" value={formData.imageUrl} onChange={handleChange}
            error={errors.imageUrl} placeholder="https://exemplo.com/imagem.jpg"
            disabled={!!imagePreview && formData.imageUrl === imagePreview} 
        />
        {imagePreview && formData.imageUrl === imagePreview && (
             <p className="text-xs text-orange-500 dark:text-orange-400">Para usar URL, remova a imagem local selecionada acima.</p>
        )}
      </div>
