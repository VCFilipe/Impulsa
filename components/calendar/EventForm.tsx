...
const EventForm: React.FC<EventFormProps> = ({
  ...
  useEffect(() => {
    if (eventToEdit) {
      setFormData({
        ...
        isEvaluable:
          eventToEdit.type === EventType.EVENTO
            ? eventToEdit.isEvaluable
            : false,
        requiresAttendance:
          eventToEdit.type === EventType.EVENTO
            ? eventToEdit.requiresAttendance
            : false,
      });
    } else {
  ...
  const handleSwitchChange = (
    name: "isEvaluable" | "requiresAttendance",
    checked: boolean
  ) => {
  ...
  const handleTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setFormData((prev) => ({
      ...
      requiresAttendance:
        newType === EventType.EVENTO ? prev.requiresAttendance : false,
    }));
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-3 p-4 border border-gray-200 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700">
        ...
        <Switch
          ...
          onChange={(checked) =>
            handleSwitchChange("requiresAttendance", checked)
          }
          disabled={!isEventoType}
        />
        {!isEventoType && (
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Opções de "Avaliável" e "Exige presença" são aplicáveis apenas para
            o tipo "Evento".
          </p>
        )}
      </div>
