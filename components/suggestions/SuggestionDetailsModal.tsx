...
import {
  ...
  Send,
  Lightbulb,
} from "lucide-react"; // Added Lightbulb
...
const SuggestionDetailsModal: React.FC<SuggestionDetailsModalProps> = ({
  ...
  const [aiCommentSummary, setAiCommentSummary] =
    useState<AiCommentSummary | null>(null);
  ...
  const loadComments = useCallback(() => {
    if (suggestion) {
      const fetchedComments = commentService.getCommentsForSuggestion(
        suggestion.id
      );
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      addToast({
        ...
        message: "Não foi possível adicionar seu comentário.",
      });
    } finally {
  ...
  const handleGenerateAiSummary = async () => {
    if (!suggestion || !GEMINI_API_KEY) {
      setAiSummaryError(
        "A funcionalidade de resumo com IA não está disponível (chave de API não configurada)."
      );
      addToast({
        type: "error",
        title: "Erro de Configuração IA",
        message: "Chave de API não configurada para resumo.",
      });
    ...
    try {
      ...
      const commentsText = comments
        .map(
          (c, index) =>
            `Comentário ${index + 1} (Autor: ${
              c.authorName || c.authorId === currentUserId
                ? "Você"
                : "Anônimo/Outro"
            }): "${c.text}"`
        )
...
`;
      ...
      addToast({
        ...
        message: "A IA analisou os comentários.",
      });
    } catch (error) {
      console.error("Error generating AI comment summary:", error);
      setAiSummaryError(
        "Ocorreu um erro ao gerar a análise dos comentários. Tente novamente."
      );
      addToast({
        type: "error",
        title: "Erro na Análise IA",
        message: "Não foi possível gerar a análise.",
      });
    } finally {
      setIsAiSummaryLoading(false);
    }
  ...
  const renderAiSummaryDisplay = () => {
    ...
    const renderList = (
      ...
      return (
        <div>
          <h5 className="text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1 flex items-center">
            {icon} {title}
          </h5>
    ...
    return (
      <div className="space-y-2.5">
        <p className="text-xs text-gray-700 dark:text-gray-200">
          <strong>Sentimento Geral:</strong> {overallSentiment}
        </p>
        {renderList(
        ...
        <p className="text-xs text-gray-700 dark:text-gray-200">
          <strong>Resumo do Engajamento:</strong> {engagementSummary}
        </p>
      </div>
    );
  };
      ...
      <div className="space-y-5 max-h-[80vh] overflow-y-auto pr-2">
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-gray-500 dark:text-gray-400 mb-3">
            <div className="flex items-center">
              {suggestion.isAnonymous ? (
                <Zap
                  size={14}
                  className="mr-1.5 text-yellow-500 dark:text-yellow-400"
                />
              ) : (
                <User
                  size={14}
                  className="mr-1.5 text-blue-500 dark:text-sky-400"
                />
              )}
              ...
              <span className="ml-1">
                {suggestion.isAnonymous
                  ? "Anônimo"
                  : suggestion.authorId || "Colaborador"}
              </span>
            </div>
            <div className="flex items-center">
              <ThumbsUp
            ...
            <div className="flex items-center">
              ...
              <span className="ml-1">
                {formatDateToDisplay(suggestion.updatedAt)}
              </span>
            </div>
          </div>
          ...
          <div className="mt-1.5 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg border border-slate-200 dark:border-slate-600 min-h-[60px] flex flex-col justify-center">
            {isAiSummaryLoading && (
              <div className="flex items-center justify-center text-sm text-sky-600 dark:text-sky-400">
                <Loader2 size={20} className="animate-spin mr-2" />A IA está
            ...
            {!isAiSummaryLoading &&
              ...
              !GEMINI_API_KEY && (
                <p className="text-xs text-orange-600 dark:text-orange-400 italic text-center">
                  A análise por IA está desativada (chave de API não
                  configurada).
                </p>
              )}
          </div>
        ...
        <div className="space-y-4 pt-2">
          <h4 className="text-md font-semibold text-gray-800 dark:text-gray-100 flex items-center">
            <MessageSquare
              size={18}
              className="mr-2 text-primary dark:text-sky-400"
            />
