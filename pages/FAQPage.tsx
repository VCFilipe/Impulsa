...
import { Plus, HelpCircle, Edit2, Trash2, Tag, Loader2, Search, X as IconX, Sparkles } from 'lucide-react'; 
...
const FAQPage: React.FC = () => {
  ...
  const [displayedFAQsCount, setDisplayedFAQsCount] = useState(INITIAL_FAQS_LOAD_COUNT);
  ...
  const handleFormSubmit = useCallback((faqData: Omit<FAQ, 'id' | 'createdAt' | 'updatedAt'> | FAQ) => {
    let newFaqId = '';
    const isEditing = 'id' in faqData;

    if (isEditing) { 
      faqService.updateFAQ(faqData as FAQ);
      addToast({ type: 'success', title: 'FAQ Atualizada!', message: `A pergunta "${faqData.question}" foi atualizada.` });
    } else { 
      const newFaq = faqService.addFAQ(faqData);
      newFaqId = newFaq.id;
      addToast({ type: 'success', title: 'FAQ Criada!', message: `A pergunta "${faqData.question}" foi criada.` });
    }
    loadFAQs(); 
    ...
    if (newFaqId) {
      ...
      setOpenAccordionIds(prev => [newFaqId, ...prev.filter(id => id !== newFaqId)]);
      setTimeout(() => {
        const element = document.getElementById(`accordion-trigger-${newFaqId}`); 
  ...
  const confirmDelete = useCallback(() => {
    if (faqToDelete) {
      ...
      addToast({ type: 'success', title: 'FAQ Excluída!', message: `A pergunta "${faqToDelete.question}" foi excluída.` });
      loadFAQs(); 
  ...
  const loadMoreFAQs = useCallback(() => {
    ...
    setTimeout(() => { 
      setDisplayedFAQsCount(prev => Math.min(prev + FAQS_TO_LOAD_ON_SCROLL, filteredFAQs.length));
...
`;
      ...
      if (foundFaqId) {
        ...
        if (found) {
          setAiFoundFAQ(found);
          addToast({type: 'success', title: 'IA Encontrou!', message: `FAQ relevante encontrada.`});
        } else {
          setAiError(`A IA sugeriu uma FAQ com ID "${foundFaqId}", mas ela não foi encontrada.`);
          addToast({type: 'warning', title: 'FAQ Não Encontrada', message: 'A IA sugeriu uma FAQ inválida.'});
        }
      } else {
        setAiFoundFAQ(null);
        setAiError("Nenhuma FAQ específica foi encontrada pela IA para sua pergunta. Tente refinar sua busca ou use os filtros manuais.");
        addToast({type: 'info', title: 'Busca IA', message: 'Nenhuma FAQ específica encontrada pela IA.'});
      }

    } catch (error) {
      console.error("Error during AI FAQ search:", error);
      setAiError("Ocorreu um erro ao tentar buscar FAQs com a IA. Por favor, tente novamente.");
      addToast({type: 'error', title: 'Erro na Busca IA', message: 'Não foi possível completar a busca inteligente de FAQs.'});
    } finally {
      setIsAiLoading(false);
    }
  };
    ...
    <AccordionItem key={faq.id} id={faq.id}>
      <AccordionTrigger>
        <div className="flex-1 flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <span className="text-md font-medium text-primary dark:text-sky-400 mr-2 text-left flex-grow break-words pr-2">{faq.question}</span>
          <div className="flex items-center mt-1.5 sm:mt-0 space-x-2 flex-shrink-0">
            <div className={`flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${getCategoryColor(faq.category)}`}>
              <Tag size={12} className="mr-1" />
              <span>{faq.category}</span>
            </div>
            {!isAiSearchResult && <DropdownMenu items={getActionItems(faq)} ariaLabel={`Ações para ${faq.question}`} />}
          </div>
        </div>
      </AccordionTrigger>
      <AccordionContent>
    ...
    <div>
      <div className="mb-2"> 
         <div className="flex flex-col sm:flex-row justify-between items-center gap-x-2 gap-y-2"> 
            ...
            <HelpCircle size={30} className="mr-3 text-primary dark:text-sky-400" />
            Perguntas Frequentes (FAQ)
            </h1>
            <Button onClick={handleAddFAQ} leftIcon={<Plus size={18}/>} className="w-full sm:w-auto">
            Nova FAQ
            </Button>
        </div>
        <p className="mt-1 text-gray-600 dark:text-gray-300">Encontre respostas rápidas para as dúvidas mais comuns dos colaboradores.</p>
      </div>
      ...
      >
        <p className="text-sm text-sky-600 dark:text-sky-400 mb-3 -mt-1">
          Faça sua pergunta e a IA tentará encontrar a FAQ mais relevante. (Experimental)
        </p>
        ...
        {isAiLoading && (
          <div className="mt-4 text-center">
            ...
            <p className="text-sm text-sky-600 dark:text-sky-400">A IA está buscando nas FAQs...</p>
          </div>
        )}
        {aiError && !isAiLoading && (
      ...
      ) : !aiQuery && !aiFoundFAQ && !isAiLoading && (
        <div className="text-center py-12">
          <HelpCircle size={48} className="mx-auto text-gray-400 dark:text-gray-500 mb-3" />
          <p className="text-xl text-gray-600 dark:text-gray-300 mb-2">Nenhuma FAQ encontrada pelos filtros manuais.</p>
          <div className="text-sm text-gray-500 dark:text-gray-400 flex items-center justify-center space-x-1">
            ...
            <Button variant="link" onClick={handleAddFAQ} className="text-sm p-0">
