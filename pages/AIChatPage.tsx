...
import { Sparkles, MessageSquare, ClipboardList, User, Send, Loader2, Trash2 } from 'lucide-react';
...
const AgentSelector: React.FC<AgentSelectorProps> = ({ prompts, selectedAgentId, onSelectAgent, isLoading }) => (
    ...
    <div className="flex-grow overflow-y-auto">
      <nav className="p-2 space-y-1">
        {prompts.map(prompt => (
          ...
          >
            <ClipboardList size={20} className="mr-3 mt-1 flex-shrink-0 text-gray-400 dark:text-gray-500" />
...
const ChatWelcome: React.FC = () => (
  <div className="flex flex-col items-center justify-center h-full text-center text-gray-500 dark:text-gray-400 p-8">
    ...
    <h2 className="text-2xl font-semibold text-gray-700 dark:text-gray-200">Bem-vindo ao Chat IA</h2>
    <p className="mt-2 max-w-md">Selecione um agente na barra lateral para iniciar uma conversa. Cada agente é especializado em uma tarefa diferente.</p>
...
interface ChatMessageViewProps { message: ChatMessage; }
  ...
  return (
    <div className={`flex items-start gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
      ...
      <div className={`flex flex-col gap-1 max-w-xl ${isUser ? 'items-end' : 'items-start'}`}>
        <div className={`p-3 rounded-lg whitespace-pre-wrap ${isUser ? 'bg-primary text-white dark:bg-sky-600 dark:text-sky-50 rounded-br-none' : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200 rounded-bl-none'}`}>
          <p className="text-sm font-normal">{message.text}</p>
        </div>
        <span className="text-xs font-normal text-gray-400 dark:text-gray-500">{new Date(message.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
      </div>
      {isUser && (
  ...
  return (
    <div className="p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 shrink-0">
      <div className="flex items-center space-x-3">
        ...
        />
        <Button onClick={onSendMessage} disabled={isLoading || !input.trim()} className="self-end" aria-label="Enviar mensagem">
...
const AIChatPage: React.FC = () => {
  ...
  useEffect(() => { setPrompts(aiPromptService.getAIPrompts()); }, []);
  ...
  const getOrCreateChatSession = useCallback((agentId: string): Chat => {
    ...
    if (!process.env.API_KEY) {
      addToast({ type: 'error', title: 'Erro de Configuração', message: 'A chave de API para IA não está configurada.' });
  ...
  const handleSendMessage = async () => {
    ...
    const userMessage: ChatMessage = { role: 'user', text: currentInput.trim(), timestamp: new Date().toISOString() };
    ...
    try {
      ...
      const modelMessage: ChatMessage = { role: 'model', text: response.text, timestamp: new Date().toISOString() };
      const finalMessages = [...updatedMessages, modelMessage];
      setChatMessages(finalMessages);
      saveChatHistory(selectedAgentId, finalMessages);
    } catch (error) {
      console.error("Error sending message to Gemini:", error);
      addToast({ type: 'error', title: 'Erro na IA', message: 'Não foi possível obter uma resposta. Tente novamente.' });
  ...
  const handleClearHistory = () => {
    if (selectedAgentId && window.confirm(`Tem certeza que deseja apagar o histórico de conversa com "${selectedAgent?.title}"?`)) {
      clearChatHistory(selectedAgentId);
      setChatMessages([]);
      setChatSessions(prev => ({ ...prev, [selectedAgentId]: null }));
      addToast({ type: 'info', title: 'Histórico Limpo', message: `A conversa com ${selectedAgent?.title} foi apagada.` });
    }
  };
  ...
  return (
    <div className="flex flex-col sm:flex-row h-[calc(100vh-6.5rem)] bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 overflow-hidden">
      ...
      <div className="w-1/3 min-w-[280px] max-w-[350px] h-full hidden sm:block">
        <AgentSelector prompts={prompts} selectedAgentId={selectedAgentId} onSelectAgent={handleSelectAgent} isLoading={isLoading} />
      </div>

      <div className="flex-1 flex flex-col h-full overflow-hidden">
        {!selectedAgentId ? <ChatWelcome /> : (
          <>
            <div className="p-3 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center shrink-0">
              <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100 truncate pr-2">{selectedAgent?.title || 'Chat'}</h2>
              <Button variant="ghost" size="sm" onClick={handleClearHistory} disabled={isLoading || chatMessages.length === 0} title="Limpar histórico da conversa">
            ...
            <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-4 space-y-4">
              {chatMessages.map((msg, index) => <ChatMessageView key={index} message={msg} />)}
              {isLoading && (
            ...
            <ChatInput input={currentInput} setInput={setCurrentInput} onSendMessage={handleSendMessage} isLoading={isLoading} />
          </>
        )}
      </div>
    </div>
