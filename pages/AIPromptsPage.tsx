...
import { Plus, ClipboardList, Edit2, Trash2, Tag, Copy, Loader2, Search, X as IconX, Wand2, Save, Sparkles } from 'lucide-react'; 
...
const AIPromptsPage: React.FC = () => {
  ...
  const [displayedPromptsCount, setDisplayedPromptsCount] = useState(INITIAL_PROMPTS_LOAD_COUNT);
  ...
  const [generatedAIData, setGeneratedAIData] = useState<GeneratedAIData | null>(null);
  const [isGeneratingWithAI, setIsGeneratingWithAI] = useState(false);
  const [aiGenerationError, setAiGenerationError] = useState<string | null>(null);
  const [formInitialData, setFormInitialData] = useState<Partial<Omit<AIPrompt, 'id' | 'createdAt' | 'updatedAt'>> | undefined>(undefined);
  ...
  const handleAddPrompt = (initialData?: Partial<Omit<AIPrompt, 'id' | 'createdAt' | 'updatedAt'>>) => {
  ...
  const handleFormSubmit = useCallback((promptData: Omit<AIPrompt, 'id' | 'createdAt' | 'updatedAt'> | AIPrompt) => {
    let newPromptId = '';
    const isEditing = 'id' in promptData;

    if (isEditing) { 
      aiPromptService.updateAIPrompt(promptData as AIPrompt);
      addToast({ type: 'success', title: 'Prompt Atualizado!', message: `O prompt "${promptData.title}" foi atualizado.` });
    } else { 
      const newPrompt = aiPromptService.addAIPrompt(promptData);
      newPromptId = newPrompt.id;
      addToast({ type: 'success', title: 'Prompt Criado!', message: `O prompt "${promptData.title}" foi criado.` });
    }
    loadPrompts(); 
    ...
    if (newPromptId) {
      ...
      setDisplayedPromptsCount(prevCount => Math.max(prevCount, newIndex + 1));
      setOpenAccordionIds(prev => [newPromptId, ...prev.filter(id => id !== newPromptId)]);
      setTimeout(() => {
        const element = document.getElementById(`accordion-trigger-${newPromptId}`);
  ...
  const confirmDelete = useCallback(() => {
    if (promptToDelete) {
      ...
      addToast({ type: 'success', title: 'Prompt Excluído!', message: `O prompt "${promptToDelete.title}" foi excluído.` });
      loadPrompts(); 
      setAiFoundPrompt(prev => prev?.id === promptToDelete.id ? null : prev);
  ...
  const uniqueUseCasesForFilter = useMemo(() => {
    return [{ value: '', label: 'Todos os Casos de Uso' }, ...AI_PROMPT_USE_CASES.map(uc => ({ value: uc, label: uc }))]
  }, []);
  ...
  const loadMorePrompts = useCallback(() => {
    ...
    setTimeout(() => { 
      setDisplayedPromptsCount(prev => Math.min(prev + PROMPTS_TO_LOAD_ON_SCROLL, filteredPrompts.length));
  ...
  const handleCopyPromptText = (textToCopy: string, title: string) => {
    navigator.clipboard.writeText(textToCopy)
      .then(() => addToast({ type: 'success', title: 'Texto Copiado!', message: `Conteúdo do prompt "${title}" copiado.` }))
      .catch(err => {
        console.error('Failed to copy text: ', err);
        addToast({ type: 'error', title: 'Falha ao Copiar', message: 'Não foi possível copiar o texto.' });
      });
  };
...
`;
      ...
      if (foundPromptId) {
        ...
        if (found) {
          ...
          addToast({type: 'success', title: 'IA Encontrou!', message: `Prompt "${found.title}" encontrado.`});
          setTimeout(() => {
            const element = document.getElementById(`accordion-trigger-${found.id}`);
            element?.scrollIntoView({ behavior: 'smooth', block: 'center' });
          }, 100);
        } else {
          setAiError(`A IA sugeriu um prompt com ID "${foundPromptId}", mas ele não foi encontrado.`);
          addToast({type: 'warning', title: 'Prompt Não Encontrado', message: 'A IA sugeriu um prompt inválido.'});
        }
      } else {
        setAiFoundPrompt(null);
        setAiError("Nenhum prompt específico foi encontrado pela IA para sua pergunta. Tente refinar sua busca ou use os filtros manuais.");
        addToast({type: 'info', title: 'Busca IA', message: 'Nenhum prompt específico encontrado pela IA.'});
      }

    } catch (error) {
      console.error("Error during AI Prompt search:", error);
      setAiError("Ocorreu um erro ao tentar buscar prompts com a IA. Por favor, tente novamente.");
      addToast({type: 'error', title: 'Erro na Busca IA', message: 'Não foi possível completar a busca inteligente de prompts.'});
    } finally {
      setIsAiLoading(false);
    }
  };
  ...
  const handleGeneratePromptWithAI = async () => {
    if (!userPromptRequest.trim()) {
      ...
      addToast({ type: 'info', title: 'Campo Vazio', message: 'Descreva o prompt antes de gerar.' });
      return;
    }
    if (!process.env.API_KEY) {
      setAiGenerationError("A funcionalidade de geração de prompt com IA não está disponível (chave de API não configurada).");
      addToast({ type: 'error', title: 'Erro de Configuração IA', message: 'Chave de API não configurada.' });
      return;
    }
...
3.  \`suggestedUseCase\`: O caso de uso MAIS apropriado da lista de "Casos de Uso Válidos" fornecida acima. Se nenhum se encaixar perfeitamente, escolha o mais próximo ou "${AI_PROMPT_USE_CASES[0] || 'Geral'}".
...
O \`generatedPromptText\` deve ser completo e pronto para uso.`;
      ...
      addToast({ type: 'success', title: 'Prompt Gerado!', message: 'A IA criou um prompt e metadados para você.' });
    } catch (error) {
      console.error("Error generating prompt with AI:", error);
      setAiGenerationError("Ocorreu um erro ao tentar gerar o prompt com a IA. Tente novamente.");
      addToast({ type: 'error', title: 'Erro na Geração IA', message: 'Não foi possível gerar o prompt.' });
    } finally {
    ...
    <AccordionItem key={prompt.id} id={prompt.id}>
      <AccordionTrigger>
        <div className="flex-1 flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <span className="text-md font-medium text-primary dark:text-sky-400 mr-2 text-left flex-grow break-words pr-2">{prompt.title}</span>
          <div className="flex items-center mt-1.5 sm:mt-0 space-x-2 flex-shrink-0">
            <div className={`flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${getUseCaseTagColor(prompt.useCase)}`}>
              <Tag size={12} className="mr-1" />
              <span className="truncate" title={prompt.useCase}>{prompt.useCase}</span>
            </div>
            {!isAiSearchResult && <DropdownMenu items={getActionItems(prompt)} ariaLabel={`Ações para ${prompt.title}`} />}
          </div>
        </div>
      </AccordionTrigger>
      <AccordionContent>
        <div className="space-y-3">
            <div>
                <h4 className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-0.5">Caso de Uso:</h4>
                <p className="text-sm text-gray-700 dark:text-gray-300">{prompt.useCase}</p>
            </div>
            <div>
                <h4 className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-0.5">Descrição:</h4>
                <p className="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap leading-relaxed">{prompt.description}</p>
            </div>
            <div>
                <div className="flex justify-between items-center mb-1">
                    <h4 className="text-xs font-semibold text-gray-500 dark:text-gray-400">Texto do Prompt:</h4>
                    <Button 
                        variant="ghost" 
                        size="sm" 
                        onClick={() => handleCopyPromptText(prompt.promptText, prompt.title)} 
    ...
    <div>
      <div className="mb-2"> 
         <div className="flex flex-col sm:flex-row justify-between items-center gap-x-2 gap-y-2"> 
            ...
            <ClipboardList size={30} className="mr-3 text-primary dark:text-sky-400" />
            Catálogo de Prompts de IA
            </h1>
            <Button onClick={() => handleAddPrompt()} leftIcon={<Plus size={18}/>} className="w-full sm:w-auto">
            Novo Prompt
            </Button>
        </div>
        <p className="mt-1 text-gray-600 dark:text-gray-300">Gerencie e explore prompts para diversas tarefas de Inteligência Artificial.</p>
      </div>
      ...
      >
        <h4 className="text-md font-medium text-sky-600 dark:text-sky-400 mb-2">
          Precisa de um prompt específico? Descreva o que você quer que a IA faça.
        </h4>
        <div className="space-y-3">
          <Textarea
            id="aiPromptQuery"
        ...
        {isAiLoading && (
          <div className="mt-4 text-center">
            ...
            <p className="text-sm text-sky-600 dark:text-sky-400">A IA está buscando nos prompts...</p>
          </div>
        )}
        {aiError && !isAiLoading && (
        ...
        {aiFoundPrompt && !isAiLoading && !aiError && (
          <div className="mt-4">
            ...
            <Accordion defaultOpenIds={[aiFoundPrompt.id]} className="space-y-0">
      ...
      >
        <h4 className="text-md font-medium text-sky-600 dark:text-sky-400 mb-2">
          Precisa de um novo prompt? Descreva a tarefa e a IA irá criá-lo para você.
        </h4>
        <div className="space-y-3">
          <Textarea
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <Button 
              ...
              leftIcon={isGeneratingWithAI ? <Loader2 size={18} className="animate-spin" /> : <Wand2 size={18} />}
              className="w-full sm:w-auto bg-sky-600 hover:bg-sky-700 dark:bg-sky-500 dark:hover:bg-sky-400 text-white"
            >
        ...
        {isGeneratingWithAI && (
          <div className="mt-4 text-center p-4 bg-gray-50 dark:bg-gray-700/50 rounded-md border border-gray-200 dark:border-gray-600">
            <Loader2 size={24} className="animate-spin text-sky-600 dark:text-sky-400 inline-block" />
            <p className="text-sm text-gray-600 dark:text-gray-300">A IA está criando seu prompt...</p>
          </div>
        )}
        ...
        {generatedAIData && generatedAIData.generatedPromptText && !isGeneratingWithAI && !aiGenerationError && (
          <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600 shadow">
            <div className="flex justify-between items-center mb-2">
                <h4 className="text-md font-semibold text-gray-800 dark:text-gray-100">Prompt Gerado pela IA:</h4>
                <Button 
                    variant="ghost" 
                    size="sm" 
                    onClick={() => handleCopyPromptText(generatedAIData.generatedPromptText, `Prompt Gerado: ${generatedAIData.suggestedTitle.substring(0,20)}...`)}
                    leftIcon={<Copy size={14} />}
                    className="text-xs px-2 py-1 text-primary dark:text-sky-400 hover:bg-primary/10 dark:hover:bg-sky-400/10"
                >
                    Copiar Prompt
      ...
      {currentlyDisplayedPrompts.length > 0 ? (
        <>
        <h3 className="text-xl font-semibold text-gray-700 dark:text-gray-200 mt-4 mb-1">Prompts Salvos no Catálogo:</h3>
        <Accordion defaultOpenIds={openAccordionIds} className="space-y-0"> 
          {currentlyDisplayedPrompts.map(prompt => renderPromptItem(prompt))}
        </Accordion>
        {hasMorePrompts && (
      ...
      ) : !aiQuery && !aiFoundPrompt && !isAiLoading && (
        <div className="text-center py-12">
          <ClipboardList size={48} className="mx-auto text-gray-400 dark:text-gray-500 mb-3" />
          <p className="text-xl text-gray-600 dark:text-gray-300 mb-2">Nenhum prompt encontrado no catálogo.</p>
          <div className="text-sm text-gray-500 dark:text-gray-400 flex items-center justify-center space-x-1">
            <span>
              {searchTerm || filterUseCase ? 'Tente ajustar seus filtros ou ' : 'Você pode '}
            </span>
            <Button variant="link" onClick={() => handleAddPrompt()} className="text-sm p-0">
