...
const AnnouncementsPage: React.FC = () => {
  ...
  const [selectedAnnouncementForDetails, setSelectedAnnouncementForDetails] =
    useState<Announcement | null>(null);

  const [displayedAnnouncementsCount, setDisplayedAnnouncementsCount] =
    useState(INITIAL_ANNOUNCEMENTS_LOAD_COUNT);
  ...
  const [aiFoundAnnouncement, setAiFoundAnnouncement] =
    useState<Announcement | null>(null);
    ...
    (
      ...
      if (isEditing) {
        ...
        addToast({
          ...
          message: `O comunicado "${data.title}" foi atualizado.`,
        });
      } else {
        ...
        addToast({
          ...
          message: `O comunicado "${data.title}" foi publicado.`,
        });
      }
  ...
  const confirmDelete = useCallback(() => {
    if (announcementToDelete) {
      ...
      addToast({
        ...
        message: `O comunicado "${announcementToDelete.title}" foi excluído.`,
      });
      loadAnnouncements(false);
      setAiFoundAnnouncement((prev) =>
        prev?.id === announcementToDelete.id ? null : prev
      );
  ...
  const handleTogglePin = useCallback(
    async (announcementId: string) => {
      ...
      if (announcement) {
        try {
          const updatedAnn = await announcementService.updateAnnouncement({
            ...announcement,
          ...
          addToast({
            ...
            message: `Comunicado ${
              updatedAnn.isPinned ? "fixado" : "desfixado"
            } com sucesso.`,
          });
          loadAnnouncements(false);
        } catch (error) {
          addToast({
            ...
            message: "Não foi possível atualizar o comunicado.",
          });
        }
      }
    },
  ...
  const handleMarkAsRead = useCallback(
    (announcementId: string) => {
      ...
      addToast({
        ...
        message: "O comunicado foi marcado como lido.",
      });
    },
    [addToast]
  );
  ...
  const handleMarkAsUnread = useCallback(
    (announcementId: string) => {
      ...
      setAllAnnouncements((prev) =>
        prev.map((a) => (a.id === announcementId ? { ...a, isRead: false } : a))
      );
      window.dispatchEvent(new CustomEvent("announcementsUpdated"));
      addToast({
        type: "info",
        title: "Marcado como Não Lido",
        message: "O comunicado foi marcado como não lido.",
      });
    },
    [addToast]
  );
  ...
  const loadMoreAnnouncements = useCallback(() => {
    ...
    setTimeout(() => {
      setDisplayedAnnouncementsCount((prev) =>
        Math.min(
          prev + ANNOUNCEMENTS_TO_LOAD_ON_SCROLL,
...
`;
      ...
      if (foundAnnouncementId) {
        const found = allAnnouncements.find(
          (f) => f.id === foundAnnouncementId
        );
        if (found) {
          ...
          addToast({
            type: "success",
            title: "IA Encontrou!",
            message: `Comunicado "${found.title}" encontrado.`,
          });
        } else {
          setAiError(
            `A IA sugeriu um comunicado com ID "${foundAnnouncementId}", mas ele não foi encontrado.`
          );
          addToast({
            type: "warning",
            title: "Comunicado Não Encontrado",
            message: "A IA sugeriu um comunicado inválido.",
          });
        }
      } else {
        setAiFoundAnnouncement(null);
        setAiError(
          "Nenhum comunicado específico foi encontrado pela IA para sua pergunta. Tente refinar sua busca ou use os filtros manuais."
        );
        addToast({
          type: "info",
          title: "Busca IA",
          message: "Nenhum comunicado específico encontrado pela IA.",
        });
      }
    } catch (error) {
      console.error("Error during AI Announcement search:", error);
      setAiError(
        "Ocorreu um erro ao tentar buscar comunicados com a IA. Por favor, tente novamente."
      );
      addToast({
        type: "error",
        title: "Erro na Busca IA",
        message:
          "Não foi possível completar a busca inteligente de comunicados.",
      });
    } finally {
      setIsAiLoading(false);
    }
    ...
    <div>
      <div className="mb-2">
        ...
        <p className="mt-1 text-gray-600 dark:text-gray-300">
          Fique por dentro das últimas notícias e avisos importantes da empresa.
        </p>
      </div>
      ...
      <FilterPanel title="Filtros" className="mb-2" initialCollapsed={true}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 items-end">
          ...
          <Select
            ...
            onChange={(e) =>
              setFilterCategory(e.target.value as AnnouncementCategory | "")
            }
            options={[
              { value: "", label: "Todas" },
        ...
        {isAiLoading && (
          <div className="mt-4 text-center">
            ...
            <p className="text-sm text-sky-600 dark:text-sky-400">
              A IA está buscando nos comunicados...
            </p>
          </div>
        )}
        {aiError && !isAiLoading && (
      ...
      {isLoadingInitial ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from({ length: INITIAL_ANNOUNCEMENTS_LOAD_COUNT }).map(
            (_, index) => (
              <div
                key={index}
                className="bg-white dark:bg-gray-800 shadow-lg rounded-xl p-5 animate-pulse"
              >
      ...
      <SidePanel
        ...
        title={
          announcementToEdit ? "Editar Comunicado" : "Criar Novo Comunicado"
        }
      >
        <AnnouncementForm
