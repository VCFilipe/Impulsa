...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = useCallback((email?: string, password?: string) => {
    // Simulação de login - em um app real, haveria uma chamada de API aqui
    console.log('Context: Login attempt with:', email, password); // Log para simulação
    try {
  ...
  const logout = useCallback(() => {
    try {
      ...
      console.error("Failed to remove isAuthenticated from localStorage:", error);
    }
